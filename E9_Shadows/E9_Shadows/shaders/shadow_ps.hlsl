...
float4 main(InputType input) : SV_TARGET
{
    ...
    float4 colour = float4(0.0f, 0.0f, 0.0f, 1.f);
    ...
    for (int i = 0; i < 2;i++)
    {
        ...
        colour1[i] = float4(0.0, 0.0, 0.0, 1.0);
    ...
    // Shadow test. Is or isn't in shadow
        ...
        // Has depth map data
            if (!isInShadow(depthMapTexture[i], pTexCoord, input.lightViewPos[i], shadowMapBias))
            {
                ...
                colour1[i] = calculateLighting(-direction[i], input.normal, diffuse[i]);
            }
        }    
    ...
    colour = saturate(colour1[0]+colour1[1] + ambient[0]);
