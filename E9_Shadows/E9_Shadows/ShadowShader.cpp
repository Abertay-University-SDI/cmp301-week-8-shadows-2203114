...
void ShadowShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX &worldMatrix, const XMMATRIX &viewMatrix, const XMMATRIX &projectionMatrix, ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView*depthMap[2], Light* light[2])
{
	...
	XMMATRIX tLightViewMatrix[2];// = XMMatrixTranspose(light[0]->getViewMatrix() + light[1]->getViewMatrix());
	XMMATRIX tLightProjectionMatrix[2];//= XMMatrixTranspose(light[0]->getOrthoMatrix() + light[1]->getOrthoMatrix());
	for (int i = 0; i < 2; i++)
	{
	...
	for (int i = 0; i < 2; i++)
	{
		...
		lightPtr->direction[i] = light[i]->getDirection();
	}
